...
> **发现 bug、改了代码、踩了坑——都要写进 [`docs/devlog/`](docs/devlog/)，
> 并随改动一起提交推送。**

不是「发版时再补」。理由很实际：日志里最值钱的部分是**当时才知道的东西**——
为什么否决了另一个方案、这个报错排查了多久、哪个「看起来能用」其实有反例。
隔一周再回忆，剩下的只有结论，而结论在代码注释里已经有了。

写什么见 [`docs/devlog/README.md`](docs/devlog/README.md) 的「写作约定」，
四条底线：**改了什么 / 为什么这么改（含放弃的方案）/ 怎么验证的 / 踩了什么坑**。

**尤其是 bug**：写根因，不要写「修了个 bug」。如果曾经把这个 bug 当成
设计取舍写进过注释或提交信息，把那次自我修正也照写进去——
它比正确的结论更能防住下一次。
