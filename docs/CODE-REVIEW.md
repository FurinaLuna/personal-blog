...
| 顺序 | 动作 | 成本 | 风险 | 收益 |
|---|---|---|---|---|
| 1 | §1.1 + §3.1-1：抽 `unique_slug()`，文章服务改为委托 `TaxonomyService.ensure_tags/ensure_category` | 低 | 低 | 消除唯一已知退化，文章服务瘦 60+ 行 |
| 2 | §4：删 `markdownToText`、收敛 `SocialLink` 导出 | 极低 | 极低 | 清理死代码 |
| 3 | §2.1 + §2.2：`emptyPage<T>()` 工厂 + 状态元数据单一来源 | 低 | 低 | 消灭两类多处同步 |
| 4 | §2.4：slug 形状配置化 + CI 断言 | 低 | 低 | 消除跨端静默耦合 |
| 5 | §3.3：`CommentSection` 迁移 + 评论分页 | 中 | 中 | 全站一致 + 修性能问题 |
| 6 | §7 配套：引入 **ESLint + jscpd（或 ruff C901 复杂度上限）**，把 `max-lines`、`max-lines-per-function` 设为 CI 门禁 | 低 | 低 | **让上面这些问题不能再悄悄出现** |
| 7 | §2.3：抽 `FormField.vue`（顺带补 `aria-invalid`） | 低 | 低 | 表单重复 + 无障碍 |
| 8 | §3.4：演示数据外置 | 低 | 低 | 可读性 |
| 9 | §3.2：拆 `ArticleEditView`（先补组件测试） | 中 | 中 | 最大单文件降到 200 行内 |
| 10 | §3.1-2：`ArticleService` 读写分离（可选） | 中 | 中 | 读路径可独立演进 |
