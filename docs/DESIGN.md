...
**2026-09-25 起：`refresh_token` 放在 httpOnly Cookie 里，`access_token` 只留在内存。**

在此之前两者都明文躺在 `localStorage`，而 `refresh_token` 是**能自我续期**的长效凭证
（7 天）—— 一次 XSS 就能把它读走，并且服务端既看不见也拦不住：攻击者可以在整个有效期
内反复换出新的 `access_token`。把它移出 JS 可读范围，这个攻击面才真正消失。
...
| `refresh_token` | httpOnly Cookie | `Path=/api/v1/auth`、`SameSite=Lax`、生产另加 `Secure`；7 天 |

**代价一：刷新页面必须静默续期。** access token 不落盘，所以每次页面加载都要先用 Cookie
换一枚新的，再问 `/auth/me`。这条路径断了就是「刷新任意页面即被登出」—— 前后端各有专门
用例守着（后端 `test_refresh_cookie.py` 的浏览器路径用例、前端 `stores/auth.spec.ts` 的启动
静默续期用例）。这也是为什么登出与改密码后**必须**能真正吊销 refresh token：迁移到 Cookie
之后，它是唯一还能被服务端撤销的凭证。

**代价二：Cookie 会被浏览器自动附带，于是要自己防 CSRF。** 用的是 `Origin` 白名单而不是
CSRF Token —— 刷新接口会校验带 `Origin` 的请求是否落在 `CORS_ORIGINS` + `SITE_BASE_URL`
内，不带 `Origin` 的直接放行（curl / 服务端脚本本来就不在 CSRF 的威胁模型里）。

> ⚠️ 由此产生一个容易漏掉的耦合：**`CORS_ORIGINS` 现在同时是安全白名单**，不只是 CORS
> 配置。分域部署（前端与 API 不同域）时前端来源必须写进去，否则刷新一律 403 ——
> 而症状不是"某个接口挂了"，是"登录用户刷新任意页面即被登出"。

为什么用白名单而不是 CSRF Token：本站只有一个前端、且同源部署是默认形态，白名单已经
覆盖了实际威胁；而 CSRF Token 需要额外的签发 / 存储 / 轮换 / 跨标签页同步，成本明显更高。
若将来出现"第三方站点需要调用这些接口"的需求，那时再补 Token 也不迟 —— 两层可以共存。

**代价三：匿名访客的一次多余请求，以及消掉它的办法。** access token 只在前端内存里，
前端**无法从 JS 侧判断**这台浏览器有没有会话，所以连公开页面也会先打一次
`POST /auth/refresh`，匿名访客每次进站白吃一个 401。解法是再下发一个**非 httpOnly** 的
「会话提示」Cookie（`blog_session`，值恒为 `"1"`）：

- 它**不是凭证**：不含 token、不含用户标识、不含过期时间，服务端**不认它**。
  它只表达"这台浏览器可能有会话"。XSS 读它没有任何额外收益 —— 攻击者想验证会话，
  直接发一个请求就知道了。
- 它的 `path` 与 refresh Cookie **刻意不同**（必须 `/`）：Cookie 可见性受 path 约束，
  收窄到 `/api/v1/auth` 的话页面 JS 读不到它，优化静默失效，还会连带丢掉公开页的登录态恢复。
  这条已用双向用例钉死，防止后人"为了对齐"改回去。
- 它**只用来省一次请求**：受保护路由仍然无条件尝试恢复登录态。因为提示与实际会话
  **可能不一致**（用户手动删过 Cookie、跨域部署读不到它等），一旦把它当判据，
  就会变成"偶发被登出"。这是本设计里最容易走错的一步。

**分域部署还需要一件事**：`SameSite=Lax` 会拦掉跨站 XHR，所以前后端不同域时必须把
`COOKIE_SAMESITE` 改成 `none`；而 `none` 强制要求 `Secure`，否则浏览器会直接丢弃该 Cookie
（表现为"登录接口 200、`set-cookie` 也在，但下一个请求就是不带"）。生产门禁会拒绝
`none` 且未启用 `Secure` 的组合。

关于 XSS 的判断没有变 —— 本站已经从架构上大幅压缩了 XSS 面（正文经 DOMPurify 白名单消毒、
评论用纯文本渲染、上传禁用 SVG/HTML、DOMPurify 禁掉内联 `style`）。这次改动的意义是：
**即使**这层防线哪天被绕过，攻击者拿到的也只是一枚 120 分钟就过期的 `access_token`，
而不是一份能续期 7 天的长效凭证。纵深防御的价值就在于此 —— 不依赖任何单层永远不出错。

**代价三：一个额外的「会话提示」Cookie（`blog_session`）。** `access_token` 不落盘意味着
前端在页面加载时**无法从 JS 侧判断这台浏览器有没有会话**，于是连公开页面也会先打一次
`POST /api/v1/auth/refresh`，匿名访客每次进站白吃一个 401。所以登录与刷新时会额外下发一个
**非 httpOnly** 的提示 Cookie（值恒为 `"1"`，不含任何秘密），前端读它决定"值不值得去续期"。

两个必须记住的约束：

- **它的 `path` 必须是 `/`，与 refresh Cookie 的 `/api/v1/auth` 刻意不同。** Cookie 可见性受
  path 约束（RFC 6265 的路径匹配）：`document.cookie` 只暴露路径是当前页面路径前缀的 Cookie。
  页面在 `/`，所以收窄到 `/api/v1/auth` 的提示 Cookie **根本读不到** —— 那样提示位会恒为
  "无会话"，反而让已登录用户在公开页丢掉登录态恢复，是净功能回归。这条已用**双向用例**钉死
  （正向用真实 path 断言读得到；反向断言 `/api/v1/auth` 的提示 Cookie 在 `/` 读不到）。
  放宽 path 的代价只是它会跟着 `/media` 这类请求一起发出去，值无信息价值。
- **它只用来省一次请求，不能用作任何授权决策。** 提示位与实际会话状态可能不一致（用户手动
  删了 Cookie、或跨域部署时读不到）。所以受保护路由仍然**无条件**尝试恢复登录态 ——
  一旦有人图快把提示位也用到那条分支，立刻会变成"偶发被登出"。

部署边界：前后端不同域且未设 `COOKIE_DOMAIN` 时，提示 Cookie 落在 API 域上，SPA 读不到它，
退化成"公开页不恢复登录态"（功能仍正确，只是顶栏慢一步）。跨子域部署请设
`COOKIE_DOMAIN=example.com`；完全不同的域之间无法共享 Cookie，这个优化在该形态下天然不可用。
...
（本条是当时的现场记录。2026-09-25 凭证已迁到 httpOnly Cookie —— 见 4.7 ——
所以"localStorage 里 token 还在"这个信号**现在不再成立**，它已不构成登录态的证据。
下面这段根因与修法本身与存储位置无关，仍然有效。）
